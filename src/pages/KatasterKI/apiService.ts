...
async function handleSubmitProfile({ json = {}, token = false }
  ...
  if (!fetchResponse.ok) {
    throw new Error(`Profile Submit Request failed: ${fetchResponse.statusText}`);
  }

  return fetchResponse.json()
}

export async function apiSubmitProfile(profileRequest: ProfileRequest): Promise<ProfileResponse> {
